package blog.anirbanm.keyboard.view;
...
public class ADFUtils {
    ...
    public static BindingContainer getBindings() {
        BindingContext bctx = getDCBindingContainer().getBindingContext();
        return bctx.getCurrent().getCurrentBindingsEntry();
    }
    ...
    public static void showPopup(final String popupId) {
        ...
        final UIComponent popup = JSFUtils.findComponentInRoot(popupId);
        if (popup != null && popup instanceof RichPopup) {
            ((RichPopup)popup).show(hints);
        } else {
            LOGGER.warning("Cannot find popup to show.");
        }
    }
